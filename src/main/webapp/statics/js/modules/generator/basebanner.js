...
var vm = new Vue({
	...
	methods: {
		...
		saveOrUpdate: function (event) {
			var url = vm.baseBanner.id == null ? "basebanner/save" : "basebanner/update";
			$.ajax({
				type: "POST",
			    url: baseURL + url,
			    contentType: "application/json",
			    data: JSON.stringify(vm.baseBanner),
			    success: function(r){
			    	if(r.code === 0){
		...
		getInfo: function(id){
			$.get(baseURL + "basebanner/info/"+id, function(r){
                vm.baseBanner = r.baseBanner;
            });
		},
